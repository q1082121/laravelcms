...
class WechatreplyimagetextController extends PublicController
{
	...
	*******************************************/
	public function api_list(Request $request)  
	{
		...
		if($keyword)
		{
			$list=Wechatreplyimagetext::where($condiiton)->where($search_field, 'like', '%'.$keyword.'%')->paginate($this->pagesize);
			//分页传参数
			$list->appends(['keyword' => $keyword,'way' =>$search_field,'wechat_id'=>$wechat_id])->links();
		}
		else
		{
			$list=Wechatreplyimagetext::where($condiiton)->paginate($this->pagesize);
