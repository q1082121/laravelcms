...
class ExperienceController extends PublicController
{
	...
	*******************************************/
	public function api_list(Request $request)  
	{
		...
		if($keyword)
		{
			$list=Experience::where($condition)->where($search_field, 'like', '%'.$keyword.'%')->orderBy('updated_at','desc')->paginate($this->pagesize);
			//分页传参数
			$list->appends(['keyword' => $keyword,'way' =>$search_field])->links();
		}
		else
		{
			$list=Experience::where($condition)->orderBy('updated_at','desc')->paginate($this->pagesize);
		}
